...
def main():
    ...
    DATA_DIR = r"F:\Base_Study\Python\数据分析\大作业\celeba_data"
    ...
    if not os.path.exists(attr_file):
        st.error(f"属性文件未找到: {attr_file}")
        st.stop()
    
    image_dir = os.path.join(DATA_DIR, 'img_align_celeba')
    if not os.path.exists(image_dir):
        st.error(f"图像目录未找到: {image_dir}")
        st.stop()
    
    # 初始化数据加载器
    data_loader = CelebADataLoader(data_dir=DATA_DIR)
